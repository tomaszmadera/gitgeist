...
2. Backend generowania obrazu (`src/gitgeist/render/image_backend.py`):
   ...
   - Implementacja `OpenAICompatibleBackend`: wywołanie HTTP API zgodnego z OpenAI Images (parametry `base_url`, `api_key`, `model`, `timeout`), zbudowane na `urllib.request` ze standardowej biblioteki, bez nowych zależności.
...
3. Fasada trybu prompt (`src/gitgeist/render/facade.py`):
   ...
   - Domyślnie (`backend=None`) używa `FakeImageBackend`; wywołanie z `OpenAICompatibleBackend` pozostaje jawną decyzją wywołującego.
...
- Żądanie HTTP do backendu używa nagłówka `Authorization: Bearer <api_key>`, treści JSON i endpointu `${base_url}/images/generations`; odpowiedź musi zawierać dane obrazu (pole `b64_json` lub URL pobierany dodatkowym żądaniem GET).
...
- `OpenAICompatibleBackend` nie loguje klucza API i nie zapisuje go w artefaktach.

## Authorization

Dostęp do backendu obrazowego kontroluje posiadacz klucza API przekazanego do `OpenAICompatibleBackend` (proponowane źródło: zmienna środowiskowa `GITGEIST_IMAGE_API_KEY`). Brak klucza przy próbie wywołania jest błędem konfiguracji, nie fallbackiem.
...
- Konfiguracja `OpenAICompatibleBackend`:
  - `base_url: str = "https://api.openai.com/v1"`
  - `api_key: str | None = None` (domyślnie ze zmiennej `GITGEIST_IMAGE_API_KEY`)
...
- Publiczne modele i funkcje:
  ...
  - `gitgeist.render.OpenAICompatibleBackend`
...
- Wywołanie `OpenAICompatibleBackend.generate_image` bez klucza API rzuca `ValueError` (brak cichego fallbacku).
...
4. `OpenAICompatibleBackend` buduje poprawne żądanie (URL, metoda, nagłówki z `Bearer`, treść JSON) bez wykonywania prawdziwego połączenia sieciowego w testach.
...
- `tests/test_image_backend.py`: kontrakt `FakeImageBackend`, budowa żądania `OpenAICompatibleBackend` (URL, nagłówki, JSON) na fasadzie `urllib`, brak klucza rzuca `ValueError`, obsługa odpowiedzi `b64_json` i błędów HTTP bez połączenia sieciowego.
